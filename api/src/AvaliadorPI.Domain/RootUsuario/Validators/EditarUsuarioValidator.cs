...
{
    public class EditarUsuarioValidator : AbstractValidator<Usuario>
    {
        private readonly IUsuarioRepository _UsuarioRepository;

        public EditarUsuarioValidator(IUsuarioRepository UsuarioRepository)
        {
        ...
        private async Task<bool> EmailUnico(Usuario usuario, string email, CancellationToken token)
        {
            return !await _UsuarioRepository.AnyAsync(x => x.Email == email && x.Id != usuario.Id);
        }
    }
}